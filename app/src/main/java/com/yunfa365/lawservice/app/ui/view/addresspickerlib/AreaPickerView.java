...
public class AreaPickerView extends RelativeLayout implements View.OnClickListener {
    ...
    private List<DiQu> mAllData;
    private List<DiQu> mRvData; // 用来在recyclerview显示的数据
    private AddressAdapter mAdapter;   // recyclerview 的 adapter

    private YwpAddressBean mYwpAddressBean; // 总数据
    private DiQu mSelectProvice; //选中 省份 bean
    private DiQu mSelectCity;//选中 城市  bean
    ...
    public AreaPickerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public AreaPickerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    public void setSelectAddress(DiQu[] address) {
        if (address == null || address.length == 0) return;

        if (address[0] != null) {
            List<DiQu> allProvince = mYwpAddressBean.getProvince();
            for (DiQu s : allProvince) {
                if (s.id == address[0].id) {
                    mSelectProvice = s;
                    mSelectCity = null;
                    ...
                    mSelectProvicePosition = allProvince.indexOf(s);
                    break;
                }
            }
        }

        if (mSelectProvice != null && address[1] != null) {
            for (int i=0; i < mSelectProvice.children.length; i++) {
                DiQu s = mSelectProvice.children[i];
                if (s.id == address[1].id) {
                    mSelectCity = s;
                    // 设置这个对应的标题
    ...
    public DiQu[] getSelectAddress() {
        return new DiQu[]{mSelectProvice, mSelectCity};
    }
    ...
    private void initData() {
        // 将数据转换为对象
        initData(YwpAddressBean.getInstance(mContext));
    }
    ...
    public void initData(YwpAddressBean bean) {
        if (bean != null) {
            ...
            mRvData.clear();
            mAllData.clear();
            for (DiQu diqu : mYwpAddressBean.getProvince()) {
                if (diqu.text.equals("浙江省")) {
                    mAllData.add(diqu);
                }
            }
            mRvData.addAll(mAllData);
        ...
        public void onTabSelected(TabLayout.Tab tab) {
            ...
            switch (tab.getPosition()) {
                ...
                case 1:
                    // 点到城市的时候要判断有没有选择省份
                    if (mSelectProvice != null) {
                        mRvData.addAll(Arrays.asList(mSelectProvice.children));
                    } else {
                ...
                case 2:
                    ...
                    if (mSelectProvice != null && mSelectCity != null) {
                        mRvData.addAll(Arrays.asList(mSelectCity.children));
                    } else {
    ...
    public interface OnAddressPickerSureListener {
        void onSureClick(String address, String provinceCode, String cityCode);
    }
