...
public class AddressPickerView extends RelativeLayout implements View.OnClickListener {
    ...
    private DiQu mSelectDistrict;//选中 区县  bean
    ...
    public void setSelectAddress(DiQu[] address) {
        ...
        if (mSelectCity != null && address[2] != null) {
            for (int i=0; i < mSelectCity.children.length; i++) {
                DiQu s = mSelectCity.children[i];
                if (s.id == address[2].id) {
                    mSelectDistrict = s;
                    // 没了，选完了，这个时候可以点确定了
    ...
    public DiQu[] getSelectAddress() {
        return new DiQu[]{mSelectProvice, mSelectCity, mSelectDistrict};
    }
    ...
    private void initData() {
        ...
        // 将数据转换为对象
        mYwpAddressBean = YwpAddressBean.getInstance(mContext);
